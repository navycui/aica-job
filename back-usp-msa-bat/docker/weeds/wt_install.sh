...
mkdir -p /logs_data/wt7/logs/ai-batch &&
mv /deploy/wt7/config/settings_sample /deploy/wt7/config/ai-batch  &&
...
sleep 5 &&
mv -f /deploy/jdbc.map  /deploy/wt7/config/ai-batch/_global/ &&
mv -f /deploy/wt7/settings.ini  /deploy/wt7/config/ai-batch/ 
