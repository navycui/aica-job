...
public class EvlOnlineService {
	...
	public PointListDto getPointList(PointListParam param) {
		...
		if(!CoreUtils.string.equals(dto.getOrgnzrId(), worker.getMemberId())) {
			throw new InvalidationException("평가위원회의 간사만 처리할 수 있습니다.");
		}
	...
	public void modifyAutoEvlSlctn(String evlCmitId) {
		...
		if(CoreUtils.string.equals(Code.evlMthd.서술형, resultList.get(0).getEvlMthdCd())) {
			throw new InvalidationException("서술형 평가방식은 자동 선정처리를 할 수 없습니다.");
		}
		...
		// 탈락기준점수
		Float drptStdrScore =  Float.valueOf(evlStep.getDrptStdrCd().substring(4));
		...
		evlTrget.setUpdatedDt(now);
		evlTrget.setUpdaterId(worker.getMemberId());
		EvlResultListDto evlResult = new EvlResultListDto();
		for(int idx=0; idx<resultList.size(); idx++) {
			evlResult = resultList.get(idx);
			if(evlStep.getSlctnTargetCo() >= evlResult.getRankRn()) {
				if(Float.compare(evlResult.getTotEvlScore(), drptStdrScore) >= 0) {
	...
	public void modifyEvlCompt(String evlCmitId) {
		...
		/* 2. 평가단계
		 * - 동일한 평가단계에서 모든 분과의 평가위원회가 완료처리가 되었을 경우
		 * - 단계별평가진행상태코드(G:EVL_STEP_STTUS) - 평가완료 처리
		 */
		UsptEvlStep evlStep = usptEvlStepDao.select(evlCmit.getEvlStepId());
		Integer evlCmitCount = usptEvlCmitDao.selectEvlCmitCountByNotEvlCompStep(evlCmitId);
		if(evlCmitCount == 0) {
			// 관련된 위원회 전체가 평가가완료된 경우
			// 평가단계 평가완료
			...
			if(usptEvlStepDao.update(evlStep) != 1) {
				throw new InvalidationException(String.format(Code.validateMessage.DB오류, "평가완료"));
			}
		}
		...
		if(evlStepList.size() == evlStep.getSortOrdrNo()) {
			...
			if(evlCmitCount == 0) {
				// 평가계획 평가완료
				UsptEvlPlan usptEvlPlan = usptEvlPlanDao.select(evlStep.getEvlPlanId());
				usptEvlPlan.setUpdatedDt(now);
				usptEvlPlan.setUpdaterId(worker.getMemberId());
				usptEvlPlan.setEvlSttusCd(Code.evlSttus.평가완료);
				if(usptEvlPlanDao.updateEvlSttusCd(usptEvlPlan) != 1) {
					throw new InvalidationException(String.format(Code.validateMessage.DB오류, "평가완료"));
				}
			}
			...
			for(SlctnListDto slctnEvlTrget : evlTrgetList) {
				...
				regEvlTrget.setEvlTrgetId(CoreUtils.string.getNewId(Code.prefix.평가대상));
