...
mkdir -p /logs_data/wt7/logs/ai-member &&
mv /deploy/wt7/config/settings_sample /deploy/wt7/config/ai-member  &&
...
sleep 5 &&
mv -f /deploy/jdbc.map  /deploy/wt7/config/ai-member/_global/ &&
mv -f /deploy/wt7/settings.ini  /deploy/wt7/config/ai-member/ 
