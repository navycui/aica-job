...
@Slf4j
public class AlrsrcStatusServiceTest extends TestServiceSupport {
	...
	@Test
	void test() {
		AlrsrcStatusParam param = AlrsrcStatusParam.builder()
										.alrsrcBgngDay(string.toDate("20220101"))
										.alrsrcEndDay(string.toDate("20221231"))
										.cmpnyNm("유기상")
										.build();

		log.info("################################################### 자원할당현황 요약 시작 #############################################");
		AlrsrcStatusSmmryDto smmryDto = service.getSummary(param);
		assertNotNull(smmryDto);
		assertTrue(smmryDto.getUseCmpnyCnt() > 0);
		log.info("################################################### 자원할당현황 요약 종료 #############################################");

		log.info("################################################### 자원할당현황 목록 시작 #############################################");
		CorePagination<AlrsrcStatusListItemDto> list = service.getList(param, new CorePaginationParam());
		assertNotNull(list);
		assertTrue(list.getTotalItems() > 0);
		log.info("################################################### 자원할당현황 목록 종료 #############################################");

		log.info(String.format("요약정보 : [%s]", json.toString(smmryDto)));
